...
# read state:
    ...
    if ape == False:
        ...
        os_url = 'https://opensea.io/assets/0x60e4d786628fea6478f785a6d7e704777c86a7c6/'+ str(id)
        print(os_url)

        if not 'listings' in response or len(response['listings']) == 0:
            print("No Listing")
            with open('MAYC-LAND2.txt', 'a') as f:
                f.write(f'\n{id}')
                f.write(', ' + 'has-land-no-listing')
                f.write(', '+ os_url)
        else:
