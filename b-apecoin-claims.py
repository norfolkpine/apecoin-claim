...
# read state:
    ...
    ape = contract_instance.functions.betaClaimed(value).call()
    strApe = str(ape)
    id = str(value)
    ...
    if ape == False:
        #id = '9130'
        print("BAYC: " + id + ", " + strApe)
        url = ('https://api.opensea.io/api/v1/asset/0xbc4ca0eda7647a8ab7c2061c2e118a18a936f13d/' + id + '/listings?format=json')
        print(url)
        ...
        os_url = 'https://opensea.io/assets/0xbc4ca0eda7647a8ab7c2061c2e118a18a936f13d/'+ str(id)

        if not 'listings' in response or len(response['listings']) == 0:
            print("No Listing")
            with open('BAYC-TEST.txt', 'a') as f:
                f.write(f'\n{id}')
                f.write(', ' + 'No Listing')
                f.write(', '+ os_url)
        else:
